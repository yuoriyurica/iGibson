...
class Instance(object):
    def __init__(self, object, id, pybullet_uuid, pose_trans, pose_rot, dynamic):
        ...
        self.next_pose_trans = pose_trans
        self.next_pose_rot = pose_rot
        self.id = id
    ...
    def render(self):
        ...
        GL.glUniformMatrix4fv(GL.glGetUniformLocation(self.renderer.shaderProgram, 'next_V'), 1,
                              GL.GL_TRUE, self.renderer.next_V)
    ...
    def set_next_position(self, pos):
        self.next_pose_trans = np.ascontiguousarray(xyz2mat(pos))

    def set_rotation(self, rot):
        self.pose_rot = np.ascontiguousarray(quat2rotmat(rot))
    
    def set_next_rotation(self, rot):
        self.next_pose_rot = np.ascontiguousarray(quat2rotmat(rot))
...
class MeshRenderer:
    def __init__(self, width=512, height=512, device_idx=0, use_fisheye=False):
        ...
        self.next_V = np.ascontiguousarray(V, np.float32)
    ...
    def set_next_camera(self, camera, target, up):
        V = lookat(camera, target, up=up)
        self.next_V = np.ascontiguousarray(V, np.float32)
    ...
    def readbuffer(self, modes=('rgb', 'normal', 'seg', 'flow')):
        results = []
    ...
    def render(self, modes=('rgb', 'normal', 'seg', 'flow'), hidden=()):
...
if __name__ == '__main__':
    ...
    renderer.instances[0].set_position([0, 0, 0])


    print(renderer.visual_objects, renderer.instances)
    print(renderer.materials_mapping, renderer.mesh_materials)
    camera_pose = np.array([0.1,0.1,0.1])
    ...
    for i in range(10000):
        ...
        if q == ord('w'):
            ...
            break
        camera_pose = np.array([px, py, 0.3])
        next_camera_pose = camera_pose + np.array([0,0.1,0.1])
        renderer.set_camera(camera_pose, camera_pose + view_direction, [0, 0, 1])
        renderer.set_next_camera(next_camera_pose, next_camera_pose + view_direction, [0, 0, 1])
